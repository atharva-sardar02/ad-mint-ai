"""
Database migration script to add title field to generations table.

This migration adds:
- title field to generations table (VARCHAR(200), nullable)

Run this script to update existing databases:
    python -m app.db.migrations.add_title_to_generations
...
def run_migration():
    """
    Run migration to add title column to generations table.
    
    This migration is idempotent - it can be run multiple times safely.
    """
    print("🔄 Starting migration: Add title field to generations table")
    ...
    try:
        if is_sqlite:
            ...
            with engine.connect() as conn:
                try:
                    conn.execute(text(
                        "ALTER TABLE generations ADD COLUMN title VARCHAR(200)"
                    ))
                    print("✅ Added title column to generations table")
                except OperationalError as e:
                    if "duplicate column name" in str(e).lower() or "already exists" in str(e).lower():
                        print("ℹ️  title column already exists, skipping")
                    else:
                        raise
            ...
            with engine.begin() as conn:
                conn.execute(text(
                    "ALTER TABLE generations ADD COLUMN IF NOT EXISTS title VARCHAR(200)"
                ))
                print("✅ Added title column to generations table (or already exists)")
