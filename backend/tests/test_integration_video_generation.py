...
from app.services.video_generation_standalone import generate_all_clips
...
async def test_complete_flow_scene_plan_to_clips_with_overlays(sample_scene_plan, tmp_path):
    ...
    with patch('app.services.video_generation_standalone.generate_video_clip') as mock_generate:
...
async def test_error_handling_api_failures_and_fallback(sample_scene_plan, tmp_path):
    ...
    from app.services.video_generation_standalone_standalone import generate_video_clip, REPLICATE_MODELS
    
    with patch('app.services.video_generation_standalone.settings') as mock_settings:
        mock_settings.REPLICATE_API_TOKEN = "test-token"
        
        # Mock Replicate client to fail primary, succeed with fallback
        with patch('app.services.video_generation_standalone.replicate.Client') as mock_client:
            client = MagicMock()
            ...
            # Mock download
            with patch('app.services.video_generation_standalone._download_video') as mock_download:
                with patch('app.services.video_generation_standalone._validate_video') as mock_validate:
                    output_dir = str(tmp_path)
                    scene = sample_scene_plan.scenes[0]
                    
                    # Should succeed with fallback model
                    from app.services import video_generation_standalone
                    clip_path, model_used = await video_generation_standalone.generate_video_clip(
                        scene=scene,
